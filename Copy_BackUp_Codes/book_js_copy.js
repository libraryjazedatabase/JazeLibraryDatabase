...
//updated closeModal tignan kung gagana
...
// update showConfrmModal tignan din kung gagana
...
// ============================================================
// 📕 MISPLACED LOGIC FIX START
// ============================================================
const grouped = {};
const misplacedBooks = [];

// Step 1: Identify misplaced books
filteredUnits.forEach(u => {
  const meta = metas[u.metadata_id];
  const preferredLoc = meta?.preferred_location || 'N/A';
  const currentLoc = u.displayLocation || 'N/A';

  // Skip borrowed and not-found books
  if (u.isBorrowed || currentLoc === 'Not Found') return;

  // If book location doesn't match preferred location → misplaced
  if (preferredLoc !== 'N/A' && currentLoc !== preferredLoc) {
    misplacedBooks.push(u);
  }
});

// Step 2: Add the Misplaced Books group (if any)
if (misplacedBooks.length > 0) {
  grouped['📕 Misplaced Books'] = misplacedBooks;
}

// Step 3: Group remaining books correctly
filteredUnits.forEach(u => {
  const meta = metas[u.metadata_id];
  const preferredLoc = meta?.preferred_location || 'N/A';
  const currentLoc = u.displayLocation || 'N/A';

  // Skip misplaced books (they’re already listed)
  ...
  // Handle books in their correct preferred location only
  else if (currentLoc === preferredLoc) {
    if (!grouped[currentLoc]) grouped[currentLoc] = [];
    grouped[currentLoc].push(u);
  }
});
// ============================================================
// 📕 MISPLACED LOGIC FIX END
// ============================================================

// LOCATION IS COMMENTED OUT BELLOW THAT CAN BE USE IN THE FUTURE
...
async function populatePreferredLocations() {
  ...
  const locationDropdown = document.getElementById('bookPreferredLocation'); // Make sure your add book form has this ID
  if (!locationDropdown) return;
  locationDropdown.innerHTML = '<option value="">Select Location</option>' + Array.from(locationSet).map(loc => `<option value="${loc}">${loc}</option>`).join('');
}
// NEW CODE END
